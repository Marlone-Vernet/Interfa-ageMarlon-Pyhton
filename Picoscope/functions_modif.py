...
def adc_rapidBlock(bufferADC, CHrange, maxADC):
    
    return True

def adc2mV(bufferADC, CHrange, maxADC):
    """ 
        adc2mc(
    ...
    vRange = channelInputRanges[CHrange]
    bufferV = [(x * vRange) / maxADC.value for x in bufferADC]

    return bufferV
	
def adc2mVpl1000(bufferADC, CHrange, maxADC):
	"""
		adc2mVpl1000(
						c_short_Array		bufferADC,
						int 				range,
	...
	bufferV = [(x * CHrange) / maxADC.value for x in bufferADC]
	
	return bufferV

def mV2adc(millivolts, CHrange, maxADC):
    """
        mV2adc(
                float                   millivolts
				int                     range
...
def mV2adcpl1000(millivolts, CHrange, maxADC):
	"""
		mV2adc(
				float				millivolts,
				int					range,
	...
	"""
	adcValue = round((millivolts * maxADC.value)/CHrange)
